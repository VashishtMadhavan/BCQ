...
class HERReplayBuffer(object):
	...
	# Expects tuples of (state, next_state, action, reward, goal, done)
	def add(self, data):
		self.storage.append(data)

	def sample(self, batch_size):
		ind = np.random.randint(0, len(self.storage), size=batch_size)
		state, next_state, action, reward, goal, done = [], [], [], [], [], []

		for i in ind: 
			s, s2, a, r, g, d = self.storage[i]
			state.append(np.array(s, copy=False))
			next_state.append(np.array(s2, copy=False))
			action.append(np.array(a, copy=False))
			reward.append(np.array(r, copy=False))
			goal.append(np.array(g, copy=False))
			done.append(np.array(d, copy=False))
